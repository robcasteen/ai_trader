...
class TestTradesEndpoint:
    """Test /api/trades/all endpoint reads from database."""

    def test_trades_endpoint_returns_db_data(self, test_db, client):
        """Test that trades endpoint returns data from database."""
        ...
        assert float(data[0]["price"]) == 50000.00

    def test_trades_endpoint_filters_test_data(self, test_db, client):
        """Test that trades endpoint filters out test mode data."""
        repo = TradeRepository(test_db)
        ...
        assert response.status_code == 200
        data = response.json()
        # Should only return production trade
        assert len(data) == 1
        assert data[0]["symbol"] == "BTCUSD"

    def test_trades_endpoint_filters_hold_actions(self, test_db, client):
        """Test that trades endpoint filters out HOLD actions."""
        repo = TradeRepository(test_db)

        # Create BUY trade
        buy_trade = repo.create(
            timestamp=datetime.utcnow(),
            action="BUY",
            symbol="BTCUSD",
            price=Decimal("50000.00"),
        ...
        # Create HOLD trade (should be filtered)
        hold_trade = repo.create(
            timestamp=datetime.utcnow(),
            action="HOLD",
            symbol="ETHUSD",
        ...
        assert response.status_code == 200
        data = response.json()
        # Should only return BUY trade, not HOLD
        assert len(data) == 1
        assert data[0]["action"] == "BUY"
...
class TestHoldingsEndpoint:
    """Test /api/holdings endpoint reads from database."""

    def test_holdings_calculated_from_db_trades(self, test_db, client):
        """Test that holdings are calculated from database trades."""
        repo = TradeRepository(test_db)
        ...
        assert float(data["holdings"]["BTCUSD"]["amount"]) == 0.1
