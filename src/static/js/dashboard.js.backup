...
function renderSymbolsTable(summary) {
  ...
  if (!keys.length) {
    container.innerHTML = '<p class="text-gray-500 text-center py-8">No trading activity yet</p>';
    return;
  }
  ...
  const rows = keys.map(sym => {
    ...
    return `
      <tr class="border-b border-gray-700 hover:bg-slate-800/50 transition-colors">
        ...
        <td class="px-4 py-3">
          <span class="signal-badge ${badgeClass}">${(d.last_action || 'N/A').toUpperCase()}</span>
        </td>
        <td class="px-4 py-3 text-gray-300">${d.last_price ? '$' + d.last_price.toLocaleString() : '-'}</td>
        <td class="px-4 py-3 ${pnlCls} font-bold">${pnlIcon} $${Math.abs(pnl).toFixed(2)}</td>
      </tr>
    `;
  }).join('');
...
function renderSentiment(sentiment) {
  ...
  if (!keys.length) {
    container.innerHTML = '<p class="text-gray-500 text-center py-4">No AI signals yet</p>';
    return;
  }
  ...
  const cards = keys.map(sym => {
    ...
    return `
      <div class="${config.bg} border-l-4 ${config.border} p-4 rounded-r-lg hover:bg-opacity-50 transition-all">
        ...
        <div class="text-gray-400 text-sm leading-relaxed">${s.reason || 'No reason provided'}</div>
        ${s.updated_at ? `
...
function renderRecentTrades(trades) {
  ...
  const items = trades.slice(-10).reverse().map(t => {
    const action = (t.action || '').toLowerCase();
    const config = actionConfig[action] || { bg: 'bg-gray-800', text: 'text-gray-400', icon: '⚪' };
    
    return `
      <div class="${config.bg} rounded-lg p-3 hover:bg-opacity-70 transition-all border border-gray-700">
        <div class="flex items-center justify-between mb-1">
          <span class="font-semibold text-white text-sm">${t.symbol || 'Unknown'}</span>
          <span class="${config.text} font-bold text-xs flex items-center gap-1">
