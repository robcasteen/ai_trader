...
# Update dashboard.py hardcoded values
DASHBOARD_FILE="src/app/dashboard.py"

# Fix line 277: "initial": 100000.0
sed -i 's/"initial": 100000\.0/"initial": 200.0/' "$DASHBOARD_FILE"

# Fix line 278: "current": 100000.0
sed -i 's/"current": 100000\.0/"current": 200.0/' "$DASHBOARD_FILE"

# Fix line 292: 100000.0 + total_pnl
sed -i 's/100000\.0 + total_pnl/200.0 + total_pnl/' "$DASHBOARD_FILE"

# Fix line 1642: default 100000
sed -i 's/"paper_starting_capital", 100000/"paper_starting_capital", 200/' "$DASHBOARD_FILE"

echo "✅ Updated dashboard.py hardcoded balances to 200"
...
if grep -q "PAPER_TRADING_BALANCE" "$CONFIG_FILE"; then
    ...
    echo "⚠️  PAPER_TRADING_BALANCE not found in config.py - manually add if needed"
fi
